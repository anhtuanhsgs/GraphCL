        ...
        if args.noisy:
            ...
            if self.multi == 1:
                ...
                for i in range (self.multi):
                    self.actor.append (outconv (features[i], out_ch, kernel_size=1))
                    self.critic.append (outconv (features[i], 1, kernel_size=1))
                    if i > 0:
...
def get_model (args, name, input_shape, features, num_actions, split, atrous_rates=[1, 6, 12, 18], gpu_id=0, multi=1):
    ...
    if name == "AttUNet2":
        model = ActorCritic (args, features , AttU_Net2 (input_shape [0], features, num_actions, split=split, level=level, multi=multi), 
